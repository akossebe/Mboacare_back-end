...
public class PrescriptionServiceImpl implements PrescriptionService {
        ...
        @Override
        public Page<PrescriptionResDTO> getTous(Pageable pageable, Long idPatient, Long idMedecin) {
            if (idPatient != null) {
                return prescriptionRepository.findByIdPatient(idPatient, pageable).map(this::versDTO);
            }
            if (idMedecin != null) {
                return prescriptionRepository.findByIdMedecin(idMedecin, pageable).map(this::versDTO);
            }
            return prescriptionRepository.findAll(pageable).map(this::versDTO);
        }

        @Override
        public PrescriptionResDTO getParConsultation(Long idConsultation) {
            Prescription prescription = prescriptionRepository.findByIdConsultation(idConsultation)
                    .orElseThrow(() -> new ResourceNotFoundException(
                            "Aucune prescription pour la consultation : " + idConsultation));
